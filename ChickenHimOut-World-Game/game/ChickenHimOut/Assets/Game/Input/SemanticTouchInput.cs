...
namespace ChickenHimOut.Game.Input
{
    ...
    {
        [SerializeField, Min(1f)] private float tapMoveThresholdPixels = 24f;
        [SerializeField, Min(0.05f)] private float tapDurationSeconds = 0.28f;
        ...
        private static DragSample ToSample(Finger finger)
        {
            ...
            return new DragSample(finger.index, touch.startScreenPosition, touch.screenPosition, touch.delta, touch.time - touch.startTime);
        }
        ...
        {
            DragSample sample = ToSample(finger);
            DragEnded?.Invoke(sample);
            if ((sample.Current - sample.Start).sqrMagnitude <= tapMoveThresholdPixels * tapMoveThresholdPixels && sample.Duration <= tapDurationSeconds)
            {
